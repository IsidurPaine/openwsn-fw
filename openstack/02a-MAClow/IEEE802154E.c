...
void     activity_ri9(PORT_RADIOTIMER_WIDTH capturedTime, OpenQueueEntry_t* msg);
...
void ieee154e_writeAsn(OpenQueueEntry_t* msg);
...
port_INLINE void activity_ri9(PORT_RADIOTIMER_WIDTH capturedTime, OpenQueueEntry_t* msg) {
	// change state
	...
	// Modifiche del 27/11/2015
	// Done: verifica che sia root il nodo
	if(idmanager_getIsDAGroot()==TRUE){
		// Done: controlla che il pacchetto sia di tipo UDP sugli ultimi 2 byte
		if(msg->payload[msg->length-1] == 255 && msg->payload[msg->length-2] == 255){
			// Done: in caso affermativo, aggiorna i 5 byte a l'asn
			ieee154e_writeAsn(msg);
		}
	}
...
// Aggiunta il 1/12/2015
port_INLINE void ieee154e_writeAsn(OpenQueueEntry_t* msg) {
	msg->payload[msg->length-1] = (ieee154e_vars.asn.bytes0and1     & 0xff);
	msg->payload[msg->length-2] = (ieee154e_vars.asn.bytes0and1/256 & 0xff);
	msg->payload[msg->length-3] = (ieee154e_vars.asn.bytes2and3     & 0xff);
	msg->payload[msg->length-4] = (ieee154e_vars.asn.bytes2and3/256 & 0xff);
	msg->payload[msg->length-5] =  ieee154e_vars.asn.byte4;
}
